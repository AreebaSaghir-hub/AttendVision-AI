...
def extract_face_gray(img_bgr, size=(200, 200)):
    ...
    return cv2.equalizeHist(cv2.resize(roi, size))


def augment(roi):
    variants = [roi, cv2.flip(roi, 1)]
    for d in [-20, 20]:
        variants.append(np.clip(roi.astype(np.int16)+d, 0, 255).astype(np.uint8))
    return variants
...
def train_recognizer():
    ...
    for s in students:
        ...
        if roi is None:
            ...
            roi  = cv2.equalizeHist(cv2.resize(gray, (200, 200)))
        for v in augment(roi):
            faces.append(v)
        ...
        if not ok:
            ...
            for (x, y, w, h) in faces:
                ...
                if recognizer is not None:
                    ...
                    if confidence < 80 and pred_label in label_to_student:
...
def register():
    if request.method == "POST":
        ...
        if not all([name, student_id, image_data]):
            return jsonify({"success": False, "message": "Name, Student ID and photo are required."})
        try:
            _, encoded = image_data.split(",", 1)
            img        = cv2.imdecode(np.frombuffer(base64.b64decode(encoded), np.uint8), cv2.IMREAD_COLOR)
            roi = extract_face_gray(img)
            if roi is None:
                return jsonify({"success": False, "message": "No face detected. Ensure good lighting."})
            conn  = get_db()
...
def attendance():
    ...
    query  = """SELECT a.*, s.department FROM attendance a
...
def attendance_chart():
    conn = get_db()
    rows = conn.execute(
        "SELECT date, COUNT(DISTINCT student_id) as cnt FROM attendance GROUP BY date ORDER BY date DESC LIMIT 14"
    ).fetchall()
    conn.close()
